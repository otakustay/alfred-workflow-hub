...
var MASTER_BRANCH_IDENTITY = {};

hub.getItem = function (repository, branch, page) {
    var itemData = null;
    if (BRANCHED_PAGES[page]) {
        var source = BRANCHED_PAGES[page];
        itemData = {
            path: branch ? source.path.replace('${branch}', branch) : source.defaultPath,
            text: source.text.replace('${branch}', branch || 'default branch'),
            icon: source.icon
        };
    }
    else if (PAGES[page]) {
    ...
    var item = {
        ...
        uid: repository + (branch ? '/' + branch : '') + '/' + page,
        valid: true,
        icon: itemData.icon,
        arg: url.replace(/\/$/, '')
    };
